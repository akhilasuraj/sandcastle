...
- 83a86f6: Add no-sandbox provider for interactive mode. `noSandbox()` runs the agent directly on the host with no container isolation — only accepted by `interactive()`, not `run()` or `createSandbox()`. The agent does not receive `--dangerously-skip-permissions`, so the user manages permissions themselves. Import from `@akhilasuraj/sandcastle/sandboxes/no-sandbox`.
...
- 16315da: Add Daytona isolated sandbox provider (`@akhilasuraj/sandcastle/sandboxes/daytona`)
...
- 5b04e73: ### Breaking changes
  ...
  - `docker()` factory is exported exclusively from `@akhilasuraj/sandcastle/sandboxes/docker`
